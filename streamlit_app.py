...
# ---------------------------------------------------------
# 2. 데이터 로드 및 전처리 (GitHub 배포용 경로 설정)
# ---------------------------------------------------------
@st.cache_data
def load_data():
    # 현재 파일(app.py)이 있는 위치를 기준으로 CSV 찾기
    current_dir = os.path.dirname(os.path.abspath(__file__))
    file_path = os.path.join(current_dir, 'festival.CSV') # 대소문자 주의!
    ...
    if '시작월' in df.columns:
        ...
        # 시작월 컬럼이 없으면 시작일에서 추출 (예: 2025-01-01)
...
# 지역별 중심 좌표 매핑 (공공데이터에 좌표가 없으므로 매핑)
lat_lon_dict = {
...
try:
    ...
    if region_col in df.columns:
        ...
        st.error("Error: Could not find 'Region' column in CSV.")
        st.stop()
...
col1.metric("Festivals Found", f"{len(filtered_df)}")
col2.metric("Selected Region", selected_region)
if not filtered_df.empty:
    top_festival = filtered_df.sort_values(by='visitors_foreign', ascending=False).iloc[0]['축제명']
...
with tab2:
    ...
    if not filtered_df.empty:
        # 보여줄 컬럼 선택
        cols_to_show = ['축제명', region_col, '개최장소', 'month', 'visitors_foreign']
        ...
        st.dataframe(
            ...
            column_config={
                "month": "Month",
                "visitors_foreign": st.column_config.NumberColumn("Foreign Visitors", format="%d")
            }
        )
    else:
        st.write("No data.")
...
with tab3:
    ...
    top10 = df.sort_values(by='visitors_foreign', ascending=False).head(10)
